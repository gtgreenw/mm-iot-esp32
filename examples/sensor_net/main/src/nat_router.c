                                   ...
                                   int32_t event_id, void *event_data)
{
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        tcpip_callback(set_backhaul_default_netif_cb, NULL);
        /* Start NTP so data log and system use correct time. */
        time_sync_start();
        /* Re-sync ESP-NOW to STA channel so C6 sensors scanning for gateway beacon find it. */
...
void start_2ghz_apsta_backhaul(void)
{
    ...
    /* AP channel 1 by default; in AP+STA mode the stack will sync AP to STA channel when STA connects. */
    wifi_config_t ap_config = {
        .ap = {
            .channel = 1,
            .max_connection = 8,
