...
typedef struct __attribute__((packed)) {
    ...
    /* S3 sensor unit (LD2410 / Seeed mmWave): presence, distances, energy, detection */
    uint8_t  mmwave_state;             /* 0=none, 1=moving, 2=stationary, 3=both */
    uint16_t mmwave_moving_cm;
    uint16_t mmwave_stationary_cm;
    uint8_t  mmwave_moving_energy;      /* 0–100 */
    uint8_t  mmwave_stationary_energy; /* 0–100 */
    uint16_t mmwave_detection_dist_cm;
} sensor_packet_t;

#define SENSOR_PACKET_SIZE sizeof(sensor_packet_t)

/* Old packet sizes for backward compat in gateway recv. */
#define SENSOR_PACKET_V5_SIZE 44
#define SENSOR_PACKET_V6_SIZE 56
/* v7 wire format (no mmwave bytes); struct size without temperature_water minus mmwave. */
#define SENSOR_PACKET_V7_WIRE_SIZE (SENSOR_PACKET_SIZE - (int)sizeof(float) - 9)
/* v8 wire size (no tds_ppm). */
#define SENSOR_PACKET_V8_SIZE (SENSOR_PACKET_SIZE - (int)sizeof(float))
/* sensor_unit_s3 (XIAO + Seeed mmWave): v6 94 bytes (legacy) or 98 bytes (with energy + detection_dist) */
#define SENSOR_PACKET_V6_S3_SIZE   98
#define SENSOR_PACKET_V6_S3_SIZE_LEGACY 94
/* S3 with moisture + plant_label (D0/D1 on XIAO) */
#define SENSOR_PACKET_V6_S3_EXT_SIZE 178
/* S3 v7: v6 ext + temperature_water + tds_ppm */
#define SENSOR_PACKET_V7_S3_SIZE 186

/* Gateway beacon (gateway → broadcast); sensors use this to lock channel during scan. */
