...
/* Serialize NVS access to the bridge namespace to avoid concurrent open/close (crash in nvs_close). */
static SemaphoreHandle_t s_nvs_mux;
...
void settings_load(bridge_settings_t *out)
{
    ...
    if (s_nvs_mux && xSemaphoreTake(s_nvs_mux, pdMS_TO_TICKS(1000)) != pdTRUE) {
        return;
    }

    nvs_handle_t h;
    if (nvs_open(SETTINGS_NS, NVS_READONLY, &h) != ESP_OK) {
        if (s_nvs_mux) xSemaphoreGive(s_nvs_mux);
        return;
    }
