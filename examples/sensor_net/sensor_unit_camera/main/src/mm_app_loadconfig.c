 ...
 *
 * Camera sensor unit: HaLow credentials from NVS (setup page) or compile-time defaults.
 */
#include "mmwlan.h"
...
/* Camera unit default HaLow credentials when not configured via NVS/setup page */
#define DEFAULT_SSID       "halow"
#define DEFAULT_PASSPHRASE "letmein111"
...
void load_mmwlan_sta_args(struct mmwlan_sta_args *sta_config)
{
	...
	if (halow_config_is_configured() && halow_config_load(ssid_buf, sizeof(ssid_buf), pass_buf, sizeof(pass_buf))) {
		...
		(void)mmosal_safer_strcpy((char *)sta_config->ssid, DEFAULT_SSID, sizeof(sta_config->ssid));
		sta_config->ssid_len = strlen((char *)sta_config->ssid);
		(void)mmosal_safer_strcpy(sta_config->passphrase, DEFAULT_PASSPHRASE, sizeof(sta_config->passphrase));
		sta_config->passphrase_len = strlen(sta_config->passphrase);
	}
