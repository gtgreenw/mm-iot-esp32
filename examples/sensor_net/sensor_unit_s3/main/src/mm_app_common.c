...
void app_wlan_init(void)
{
	...
	if (status != MMWLAN_SUCCESS)
		printf("Warning: mmwlan_set_power_save_mode failed (%d)\n", (int)status);
	/* HaLow TX power override from NVS (sensor unit settings page). 0 = regulatory default. */
	{
		nvs_handle_t h;
		if (nvs_open("sensor", NVS_READONLY, &h) == ESP_OK) {
			...
			if (nvs_get_u16(h, "halow_txp", &halow_txp) == ESP_OK && halow_txp != 0) {
				...
				if (status != MMWLAN_SUCCESS)
					printf("Warning: HaLow TX power override failed (%d)\n", (int)status);
				else
					printf("HaLow TX power set to %u dBm\n", (unsigned)halow_txp);
			}
			nvs_close(h);
		}
	}
